...
`currency-lang-translator` is a Chrome Manifest V3 extension that converts detected prices in the DOM into the user's selected currency.
...
- `background.js` as the service worker for network calls, rate limiting, caching, and alarms
...
- User-facing examples may include non-English text only when needed to demonstrate currency detection behavior, but the explanation around them should stay in English.
...
- [background.js](/Users/pmfb/Documents/coding/chrome-workspace/currency-lang-translator/background.js): exchange-rate fetches, persisted cache, alarms, and cross-context messaging
...
- Never send page text outside the browser.
- If privacy is unclear, choose the more conservative behavior.
...
- Put limits on observers, caches, retries, and batch work.
...
4. Review side effects on rate limiting and DOM duplication.
...
- NEVER send page text to an external API.
...
3. No page text is sent to external services.
...
- Do not introduce new sources of double conversion.

### `background.js`

- Centralize fetch, cache, retries, and backoff here.
- Any new timeout or retry must be bounded.
- If you change caches or alarms, think about worker suspension and safe recovery.
...
- does not violate the no-page-text-exfiltration rule
- passes syntax checks
- passes a reasonable manual test in Chrome

## Incorrect vs Correct

- Incorrect: call external rates APIs directly from `content.js`.
- Correct: keep network access in `background.js` and never send page text outside the browser.
